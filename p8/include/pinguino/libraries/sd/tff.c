...
FRESULT sync (void)		/* FR_OK: successful, FR_RW_ERROR: failed */
{
	...
	if (disk_ioctl(CTRL_SYNC, NULL) != RES_OK)
		return FR_RW_ERROR;
	return FR_OK;
}
#endif
...
//	char fn[8+3+1];
	...
	for (n = 1; n < dj.fs->csize; n++) {
		if (disk_write(fw, ++dsect, 1) != RES_OK)
			return FR_RW_ERROR;
	}
...
)
{
	...
	if (disk_ioctl(GET_SECTOR_SIZE, &SSZ(fs)) != RES_OK
		|| SSZ(fs) > S_MAX_SIZ
		|| SSZ(fs) > allocsize)
		return FR_MKFS_ABORTED;
#endif
    ...
    rc=disk_ioctl(GET_BLOCK_SIZE, &n);
	...
	return (disk_ioctl(CTRL_SYNC, NULL) == RES_OK) ? FR_OK : FR_RW_ERROR;
}
...
FRESULT f_forward (
	FIL *fp, 						/* Pointer to the file object */
	word (*func)(const byte*,word),	/* Pointer to the streaming function */
