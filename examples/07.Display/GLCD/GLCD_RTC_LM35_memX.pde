...
void loop(){ // run over and over again	
	...
	if(dia<10) GLCD.Puts("0");
	GLCD.PrintNumber(dia);
	GLCD.Puts("/");
	if(mes<10) GLCD.Puts("0");
	GLCD.PrintNumber(mes);
	GLCD.Puts("/20");
	if(ano<10) GLCD.Puts("0");
	GLCD.PrintNumber(ano); //Ano

	//Display Mode
	GLCD.GotoXY(38,56);
	switch (mode){
			case 1:
				GLCD.Puts("set Segundos");
				break;
			case 2:
				GLCD.Puts("set Minutos");
				break;
			case 3:
				GLCD.Puts("set Hora");
				break;
			case 4:
				GLCD.Puts("set Dia Semana");
				break;
			case 5:
				GLCD.Puts("set Dia");
				break;
