...
public class Font {
	...
	public Font(int width, int initWidth, int height, int startIdx, int charCount, String name, int bits, java.awt.Font importFont) {
		chars = new Vector(charCount);
		this.width = width;
		this.height = height;
		this.name = name;
		...
		for(int i=0; i<charCount; i++) {
			...
			if(importFont != null) {
				...
				if(cropTop > rect.y) cropTop = rect.y;
				if(cropBottom > rect.height) cropBottom = rect.height;
			}
			chars.addElement(c);
		}
		...
		if(importFont != null) {
			for(int i=0; i<charCount; i++) {
				((FontCharacter)(chars.get(i))).setSize(-cropTop, -cropBottom, 0, 0);
			}
		}
	}
	...
	public int getSize() {
		...
		for(int i=0; i<chars.size(); i++) {
			FontCharacter fc = (FontCharacter)chars.get(i);
			size += fc.getWidth()*fc.getHeight()*bits;
		}
